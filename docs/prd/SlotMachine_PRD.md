...
────────────────────────────────────────────
 #  문서명                    분류          확신도  근거
────────────────────────────────────────────
 1  앱_출시_체크리스트.md     Projects ✅   95%    마감 기한과 구체적 태스크 포함
 2  독서노트_원칙.md           Resources ✅  90%    참고 자료성 내용, 시간 제한 없음
 3  아이디어_메모.md           Inbox  ⚠️    45%    내용이 모호하여 직접 확인 필요
 4  건강관리_루틴.md           Areas ✅      88%    지속적 관리 영역
 5  2023_프로젝트A_회고.md    Archives ✅   92%    완료된 프로젝트
 6  클린코드_요약.md           Resources ✅  87%    기술 참고 자료
 7  오늘할일.md                Projects ✅   78%    진행 중인 작업 목록
...
✅ 6개 문서 이동 완료. git commit: "chore: PARA classify 6 inbox items [SlotMachine]"
```
...
| F4-03 | 관련도 임계값(threshold) 이상의 문서만 링크 후보로 제안 | Must |
| F4-04 | 링크 삽입 위치 추천 (관련 문맥 주변 삽입) | Should |
| F4-05 | 이미 존재하는 링크 중복 삽입 방지 | Must |
| F4-06 | 양방향 링크 처리 (A→B 링크 시 B 문서에도 backlink 반영 여부 옵션) | Should |
